...
export default function HeroSlider({ slides }) {
  ...
  if (slides.length === 0) {
    return (
      <section className="relative w-full h-[460px] overflow-hidden bg-primary-soft">
        <div className="relative z-10 h-full max-w-7xl mx-auto px-4 md:px-8 flex flex-col justify-center text-gray-900">
          <p className="uppercase tracking-[0.25em] text-accent text-xs font-bold mb-4">
            Welcome to {settings.site_title || 'the Shop'}
          </p>
          <h1 className="text-[42px] md:text-[56px] leading-[1.05] font-extrabold max-w-2xl tracking-tight">
            {settings.site_tagline || 'Quality products at great prices.'}
          </h1>
          <Link
            to="/search"
            className="self-start mt-8 inline-flex items-center gap-2 bg-primary text-white px-8 py-3.5 rounded-full font-semibold hover:bg-primary-dark transition-colors"
          >
            Start shopping
          </Link>
        </div>
  ...
  return (
    <section
      className="relative w-full h-[440px] md:h-[640px] overflow-hidden bg-gray-900"
        ...
        return (
          ...
          >
            ...
            <div className="absolute inset-0 z-10 max-w-7xl mx-auto px-4 md:px-8 flex flex-col justify-center text-white">
              {s.subtitle ? (
                <p className="uppercase tracking-[0.3em] text-accent text-xs font-bold mb-4">{s.subtitle}</p>
              ) : (
                settings.site_tagline && (
                  <p className="uppercase tracking-[0.3em] text-accent text-xs font-bold mb-4">{settings.site_tagline}</p>
                )
              )}
              {s.title ? (
                <h1 className="text-[40px] md:text-[56px] leading-[1.05] font-extrabold max-w-2xl tracking-tight [text-shadow:0_2px_24px_rgba(0,0,0,0.45)]">
                  {s.title}
                </h1>
              ) : (
                settings.site_title && (
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link
                  to={s.link || '/search'}
                  className="inline-flex items-center gap-2 bg-white text-gray-900 px-8 py-3.5 rounded-full font-bold shadow-lg hover:bg-primary hover:text-white transition-colors"
                >
